...
class PayloadRegistryTest {
    ...
    @Test
    @DisplayName("Reject null message")
    void rejectNullMessage() {
        InvalidMessageException ex = assertThrows(InvalidMessageException.class,
            () -> PayloadRegistry.deserialize(null));
        assertEquals("MALFORMED_JSON", ex.getCode());
    }

    // ── Denial-of-Service Hardening Tests ───────────────────────────────────

    @Test
    @DisplayName("Reject message exceeding the maximum allowed size")
    void rejectOversizedMessage() {
        String hugeMoves = "e".repeat(PayloadRegistry.MAX_MESSAGE_BYTES);
        String json = "{\"type\":\"ANALYZE_GAME\",\"payload\":{\"moves\":\"" + hugeMoves + "\"}}";

        InvalidMessageException ex = assertThrows(InvalidMessageException.class,
            () -> PayloadRegistry.deserialize(json));
        assertEquals("MESSAGE_TOO_LARGE", ex.getCode());
    }

    @Test
    @DisplayName("Reject pathologically deep JSON nesting")
    void rejectExcessiveNesting() {
        StringBuilder json = new StringBuilder("{\"type\":\"ANALYZE_GAME\",\"payload\":{\"moves\":\"x\"},\"extra\":");
        json.append("[".repeat(5_000));
        json.append("]".repeat(5_000));
        json.append("}");

        InvalidMessageException ex = assertThrows(InvalidMessageException.class,
            () -> PayloadRegistry.deserialize(json.toString()));
        assertEquals("MALFORMED_JSON", ex.getCode());
    }
